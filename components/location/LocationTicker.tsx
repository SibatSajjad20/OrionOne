...
const LOCATION_TAGLINE =
  "Sector F · DHA Phase III Islamabad · Beside Lakeview Commercial · Where the Lake Meets Living · Signal-Free Metropolitan Connectivity · Waterfront Living & Modern Infrastructure";

export default function LocationTicker({ className = "" }: LocationTickerProps) {
  const items = Array.from({ length: 4 });

  return (
    <section
      aria-label="Location Essence Strip"
    ...
    >
      ...
      <div className="flex w-max animate-marquee-ltr hover:[animation-play-state:paused] cursor-default">
        ...
        <div className="flex shrink-0 items-center">
          {items.map((_, i) => (
            <div key={`loc-part1-${i}`} className="flex items-center">
        ...
        <div className="flex shrink-0 items-center" aria-hidden="true">
          {items.map((_, i) => (
            <div key={`loc-part2-${i}`} className="flex items-center">
              <span className="font-serif-heading text-lg sm:text-2xl lg:text-3xl font-light text-[#EDE5DA]/90 tracking-wide whitespace-nowrap">
                {LOCATION_TAGLINE}
              </span>
              <span
